...
#define TAG "t.twitter"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TTwitter_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TTwitter.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TTwitterBindings::lookupGeneratedInit(
...
static void TTwitter_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TTwitter_getBinding)->GetFunction());
}

static void TTwitter_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TTwitterBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TTwitterBinding = {
	"t.twitter",
	TTwitter_init,
	TTwitter_dispose
};
...
Java_t_twitter_TTwitterBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("t.twitter", &TTwitterBinding);
	titanium::KrollBindings::addExternalLookup(&(::TTwitterBindings::lookupGeneratedInit));
}
